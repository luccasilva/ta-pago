describe("Home Page", () => {
  ...
  it("should navigate to Exercise page when 'Meus Exercícios' card is clicked", () => {
    cy.get("a[href='/exercise']").click();
    cy.url().should("include", "/exercise");
  });

  it("should navigate to Record page when 'Minhas Fichas' card is clicked", () => {
    cy.get("a[href='/record']").click();
    cy.url().should("include", "/record");
  });

  it("should navigate to Create Group page when '+' button is clicked", () => {
    cy.get("button").contains("+").click();
    cy.url().should("include", "/group/create");
  });

  it("should should navigate to the user group page", () => {
    cy.get("button").contains("+").click();
